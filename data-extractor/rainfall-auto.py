...
def generate_report(report_human_date,file):
    ...
    print(f'Thread created for {report_human_date} successfully with id {thread.id} and run id {run.id}')
...
##create file
file =  client.files.create(file=open('./csv/long-rainfall-data.csv', 'rb'), purpose='assistants')
