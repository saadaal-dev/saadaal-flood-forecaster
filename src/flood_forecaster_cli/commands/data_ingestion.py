...
@click.option(
    "--dry-run", is_flag=True, help="If set, only prints the duplicates without deleting them. See [HOTFIX-DUPLICATES-HISTORICAL-WEATHER]."
)
@common_options
...
@common_options
def fetch_river_data_from_csv(
    configuration: Config, location_name: str, snrfa_file: Optional[str] = None, swalim_file: Optional[str] = None
):
    """
...
#           This is a quick fix to remove duplicates from the database
#           [HOTFIX-DUPLICATES-HISTORICAL-WEATHER]
@data_ingestion.command(
    "remove-duplicates-historical-weather", help="INTERNAL: Remove duplicate historical weather entries from the database"
)
