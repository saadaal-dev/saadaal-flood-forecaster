...
def fetch_historical(config: Config, openmeteo):
    ...
    logger.debug("Fetching the latest date from the database...")
    conn = DatabaseConnection(config)

    max_date = conn.get_max_date(HistoricalWeather)
    if max_date:
        logger.debug(f"Latest historical weather date in the database: {max_date.strftime('%Y-%m-%d')}")
    ...
    if historical_df is None:
        logger.debug("No historical weather data fetched.")
        return None
...
def get_historical_weather(location_labels: List[str],
    ...
    logger.debug(f"DEBUG: max_date: {max_date}, start_date: {start_date}, end_date: {end_date}")

    if start_date > end_date:
        logger.debug("You have the most updated data in the historical weather :)")
        return None
