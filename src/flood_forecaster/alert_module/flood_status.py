...
def get_df_by_date(db_client, date_begin: datetime, risk_level="full") -> pd.DataFrame:
    ...
    if result_df.empty:
        ...
        return pd.DataFrame(columns=["location_name", "flood_risk", "water_level_m", "predicted_flood_date"])
    ...
    alert_table = alert_table.rename(
        columns={"location_name": "Station", "risk_level": "Flood risk", "level_m": "Water level (m)", "forecast_date": "Prediction date"}
    )
