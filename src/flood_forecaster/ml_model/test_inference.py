...
class TestInference(unittest.TestCase):
    def setUp(self):
        ...
        self.station_metadata = StationMapping(
            **{"location": "S1", "river": "R", "upstream_stations": ["S1", "S2"], "weather_locations": ["W1", "W2"]}
        )
        self.now = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
        self.stations_df = StationDataFrameSchema(
            pd.DataFrame(
                {
        ...
        self.weather_df = WeatherDataFrameSchema(
            pd.DataFrame(
                {
    ...
    def test_infer_naive(self):
        ...
        station_metadata = StationMapping(**{"location": "S1", "river": "R", "upstream_stations": ["S1"], "weather_locations": ["W1"]})
        stations_df = StationDataFrameSchema(
            pd.DataFrame({"location": ["S1"] * 2, "date": [self.now, self.now - timedelta(days=1)], "level__m": [0.0, 1.0]})
        )
        weather_df = WeatherDataFrameSchema(
            pd.DataFrame({"location": ["W1"], "date": [self.now], "precipitation_sum": [0.1], "precipitation_hours": [1.1]})
        )
        station_lag_days = [1]
        weather_lag_days = [0]
        ...
        expected_df = pd.DataFrame(
            {
                ...
                "lag01__level__m": [1.2],
                "lag03__level__m": [1.0],
                "s1__lag01__level__m": [1.2],
                "s2__lag01__level__m": [2.2],
                "s1__lag03__level__m": [1.0],
                "s2__lag03__level__m": [2.0],
                "w1__lag01__precipitation_sum": [0.3],
                "w1__lag01__precipitation_hours": [3.0],
                "w1__lag03__precipitation_sum": [0.1],
                "w1__lag03__precipitation_hours": [1.0],
                ...
                "w2__lag01__precipitation_sum": [0.3],
                "w2__lag01__precipitation_hours": [13.0],
                "w2__lag03__precipitation_sum": [0.1],
                "w2__lag03__precipitation_hours": [11.0],
