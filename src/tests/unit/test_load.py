...
from flood_forecaster.data_ingestion.load import load_inference_river_levels
...
class TestLoadFunctions(unittest.TestCase):
            ...
            pd.DataFrame(
                {
                    ...
                    "date": [now, now + timedelta(days=1), now, now + timedelta(days=1)],
    ...
    def test_load_inference_river_levels(self, mock_load_river_level):
        ...
        placeholder_df = pd.DataFrame({"location": ["loc1", "loc2"], "date": [now, now], "level__m": [0.0, 0.0]})
        ...
        mock_load_river_level.assert_called_with(
            mock_config, ["loc1", "loc2"], now.date() - timedelta(days=3), now.date() - timedelta(days=1), fill_missing_dates=True
        )
